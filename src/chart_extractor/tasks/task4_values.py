...
from chart_extractor.tasks import task4_exporters as t4_exporters
from chart_extractor.tasks import task4_geometry as t4_geometry
from chart_extractor.tasks import task4_matching as t4_matching
...
def extract_patch_embedding(
    ...
    if kind == "legend":
        x1, y1, x2, y2 = t4_matching.shrink_legend_bbox((x1, y1, x2, y2), (W, H))
    elif kind == "bar":
...
def getYVal(IMG_DIR, JSON_DIR, objects_dectector, backbone):
    ...
    for index, (path, r) in enumerate(
        ...
        image, x_tick_list, x_title, y_tick_list, y_title, legend_text_boxes, image_text = \
            t4_geometry.getProbableLabels(image, task2_3, xaxis, yaxis)

        # ================= GET RATIO ==========================================
        list_text, normalize_ratio, (min_val, min_pixel) = t4_geometry.getRatio_optimized(y_tick_list)
        ...
        try:
            ...
            if not NO_LEGEND:
                assignments = t4_matching.assign_legend_patches(
            ...
            for (lx, ly, lw, lh) in legendrects:
                ...
                try:
                    ...
                    sx1, sy1, sx2, sy2 = t4_matching.shrink_legend_bbox(bbox_xyxy, (img_width, img_height))
            ...
            if legend_embs_list and bar_embs_list and len(legend_embs_list) == len(legendtexts):
                try:
                    ...
                    matches = t4_matching.match_legend_to_bars(legend_embs, bar_embs)
            ...
            for rectBox in bar_rects:
                ...
                for text, textBox in x_tick_list:
                    d = t4_geometry.RectDist(rectBox, textBox)
                    if d < min_distance:
                        min_distance = d
                        closestBox = textBox
                        labeltext = text
            ...
            nd = t4_geometry.infer_ndigits_from_ticks(y_tick_list, default=1, cap=3)
